...
// ── Each registered schema row owns its own test state ────────────────────────
...
function SchemaRow({ schema, onDelete, deleting, onRequest }: SchemaRowProps) {
  ...
  const handleTestViolation = async () => {
    ...
    try {
      ...
      if (result.status === 422 && 'violations' in result) {
        ...
        setTestResult({ status: 202, message: '202 accepted (warn mode — violation logged as metric)' })
        onRequest?.({
          method: 'POST',
          path: `/v1/events (schema ${schema.mode})`,
          status: 202,
        ...
        setTestResult({ status: 202, message: `Unexpected ${result.status} response` })
      }
    } catch {
  ...
  return (
    <div className="rounded-md border border-border bg-card/40 px-3 py-2.5">
      <div className="flex items-center gap-2">
        <span className="flex-1 font-mono text-xs text-foreground">{schema.event_name}</span>
        <Badge
          variant="outline"
          className={cn(
            'shrink-0 text-[10px]',
        ...
        <button
          type="button"
          onClick={() => onDelete(schema.event_name)}
          disabled={deleting}
          ...
          aria-label={`Delete schema for ${schema.event_name}`}
        >
      ...
      <div className="mt-2">
        <button
          type="button"
          onClick={() => void handleTestViolation()}
          disabled={testing}
...
export function SchemaPanel({ onRequest }: SchemaPanelProps) {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      setRegisterError('Invalid JSON — check your schema syntax')
  ...
  const handleDelete = async (name: string) => {
    ...
    try {
      ...
      setRegisterError(`Failed to delete schema for ${name}`)
    } finally {
      setDeleting(null)
    }
  }
          ...
          <div className="space-y-2">
            {schemas.map((schema) => (
              <SchemaRow
                ...
                deleting={deleting === schema.event_name}
                onRequest={onRequest}
              />
            ))}
          </div>
